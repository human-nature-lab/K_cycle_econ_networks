...
#--------------- 1: Friendship individual networks ----------------
#Wave 1
nodelist_w1 <- conn_w1 %>%
...
friend_ind_edgelist_w1 <- conn_w1 %>%
  subset(same_village==1 & relationship %in% c("closest_friend","personal_private","free_time"),
...
friend_ind_edgelist_w2 <- conn_w2 %>%
  subset(same_village==1 & relationship %in% c("closest_friend","personal_private","free_time"),
...
network_factors_vil_w1 <- foreach(sampleNr=1:village_range, .combine=rbind) %dopar% {
  ...
  if(sampleNr%in%nodelist_w1$village_code_w1){
    print(compute_network_metrics_w1(nodelist_w1,friend_ind_edgelist_w1,sampleNr)) 
  }
...
network_factors_vil_w2 <- foreach(sampleNr=1:village_range, .combine=rbind) %dopar% {
  ...
  if(sampleNr%in%nodelist_w2$village_code_w2){
    print(compute_network_metrics_w1(nodelist_w2,friend_ind_edgelist_w2,sampleNr)) 
  }
  #print(compute_network_metrics_w1(nodelist_w2,friend_ind_edgelist_w2,sampleNr))
...
pdf('regression_items_friend.pdf',width=9,height=7)
...
pdf('figure_path/figure_1a_friend.pdf',width=5,height=6)#was ind_phen[4:6]
...
pdf('regression_items_rev_friend.pdf',width=9,height=7)
...
pdf('figure_path/figure_1b_friend.pdf',width=5,height=6)#was ind_phen[4:6]
...
for(i in 1:length(unique(cyc_comp_w13_l2$phen))){
  ...
  #pdf('comp_delta_w_kcy3_friend.pdf',width=11,height=8)
  #pdf(paste0('comp_del_',name,'_friend.pdf'),width=11,height=8)
  ...
  ggsave(paste0('figure_path/comp_del_ind_',name,'_friend.pdf'),width=11,height=8)
  #dev.off()
...
ggsave('figure_path/fig_3d_manhattan_friend.pdf',width=7,height=5)
