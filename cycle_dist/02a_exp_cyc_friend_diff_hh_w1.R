...
vil_indiv_friend_w1_diff_hh_kcy2<-array(NA,dim=c(length(vc),100))
vil_indiv_friend_w1_diff_hh_kcy3<-array(NA,dim=c(length(vc),100))
vil_indiv_friend_w1_diff_hh_kcy4<-array(NA,dim=c(length(vc),100))
vil_indiv_friend_w1_diff_hh_kcy5<-array(NA,dim=c(length(vc),100))

library(sna, quietly = TRUE)

for(j in 1:176){
  edgelist_subset <- subset(friend_diff_hh_edgelist_w1, village_code_w1==j)
  ...
  for(kk in 1:100) {
    ...
    vil_indiv_friend_w1_diff_hh_kcy2[j,kk] <- apot.g$cycle.count[1,1]
    vil_indiv_friend_w1_diff_hh_kcy3[j,kk] <- apot.g$cycle.count[2,1]
    vil_indiv_friend_w1_diff_hh_kcy4[j,kk] <- apot.g$cycle.count[3,1]
    vil_indiv_friend_w1_diff_hh_kcy5[j,kk] <- apot.g$cycle.count[4,1]
  }
  print(j)
}
